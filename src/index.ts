...
(function() {
	...
	const elementMatchesSelector =
		...
		function(selector) {
			const matchedNodes = (<Element>this.parentNode || this.document).querySelectorAll(selector);
	...
	function generateDistanceFunction(targetRect: any) {
		return {
			nearPlumbLineIsBetter: (rect: any) => {
				let d;
			...
			nearTargetLeftIsBetter: (rect: any) => {
				let d;
				if (rect.center.x < targetRect.center.x) {
			...
			nearTargetTopIsBetter: (rect: any) => {
				let d;
				if (rect.center.y < targetRect.center.y) {
			...
			topIsBetter: (rect: any) => {
				return rect.top;
			},
			bottomIsBetter: (rect: any) => {
				return -1 * rect.bottom;
			},
			leftIsBetter: (rect: any) => {
				return rect.left;
			},
			rightIsBetter: (rect: any) => {
				return -1 * rect.right;
			},
		};
	}
	...
	function focusElement(elem?, sectionId?, direction?) {
		...
		const silentFocus = function() {
			if (currentFocusedElement) {
				(currentFocusedElement as HTMLInputElement).blur();
			}
			elem.focus();
	...
	const SpatialNavigation = {
		...
		set: function(config, sectionId?: string) {
		...
		makeFocusable: function(sectionId) {
			const doMakeFocusable = function(section) {
				const tabIndexIgnoreList = section.tabIndexIgnoreList !== undefined ? section.tabIndexIgnoreList : GlobalConfig.tabIndexIgnoreList;
